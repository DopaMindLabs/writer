...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitivePortal>
    ...
    <DialogPrimitiveContent
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 border border-rule bg-paper p-6 shadow-overlay-dialog duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
        className,
      )}
      {...props}
    >
      {children}
      <DialogPrimitiveClose className="absolute right-4 top-4 opacity-70 ring-offset-paper transition-opacity hover:opacity-100 focus:outline-none focus:ring-1 focus:ring-ink">
        <X className="h-4 w-4" />
