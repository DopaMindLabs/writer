...
export async function readSpaceSnapshot(spaceId: string): Promise<SpaceSnapshot> {
...
export function backupFilename(spaceName: string, when: number): string {
  const d = new Date(when);
  const stamp = `${d.getUTCFullYear()}-${pad2(d.getUTCMonth() + 1)}-${pad2(
    d.getUTCDate(),
...
function yamlValue(value: string | number | boolean | null | undefined): string {
...
function renderSpaceMd(snapshot: SpaceSnapshot, when: number): string {
...
function buildSectionPaths(sections: Section[]): Map<string, SectionPathInfo> {
  const byId = new Map(sections.map((s) => [s.id, s]));
  const slugBySection = new Map<string, string>();
  const usedAtParent = new Map<string, Set<string>>();

  function uniqueSlug(parentKey: string, label: string, fallback: string): string {
...
function renderDocMd(doc: Doc, sectionLabel: string | undefined): string {
...
function renderNotesMd(notes: Note[]): string {
  if (notes.length === 0) {
    return '# Notes\n\n_No notes._\n';
  }
  ...
  for (const [kind, list] of [...byKind.entries()].sort()) {
    lines.push(`## ${kind}`, '');
    for (const n of list) {
      const heading = n.title?.trim() || n.body.split('\n')[0]?.trim() || '_(empty)_';
      lines.push(`- **${heading}**  \`${n.id}\``);
...
function renderCitationsMd(citations: Citation[]): string {
  ...
  for (const c of sorted) {
    lines.push(
      `- \`${c.key}\` — ${c.authors || '(unknown)'} (${c.year || 'n.d.'}). *${c.title || '(untitled)'}*. [${c.type}]`,
    );
  }
  lines.push('');
  return lines.join('\n');
...
function renderConnectionsMd(
  ...
  for (const n of notes) {
    labelFor.set(n.id, n.title?.trim() || n.body.split('\n')[0]?.trim() || n.id);
  }
...
function renderAnnotationsMd(
  ...
  for (const [docId, list] of byDoc) {
    ...
    for (const a of list) {
      const range = `${a.rangeStart}–${a.rangeEnd}`;
...
function renderPaletteMd(palettes: HighlightPalette[]): string {
  if (palettes.length === 0) return '# Palette\n\n_No palette configured._\n';
  const lines = ['# Palette', ''];
  for (const p of palettes) {
    lines.push(`## \`${p.id}\``, '');
    p.slots.forEach((slot, i) => {
      lines.push(`${i + 1}. ${slot.name} — \`${slot.color}\``);
    });
    lines.push('');
  }
  return lines.join('\n');
...
export async function buildSpaceMarkdownZip(
...
export async function buildSpaceMarkdownZipFor(
  spaceId: string,
  when: number = Date.now(),
