# Accessibility

Works with keyboards, screen readers, and your system accessibility settings.
Open **Settings → Accessibility** to tune the interface. Every option starts
at the default, so nothing changes until you choose it.

## At a glance

- **Theme & contrast** — light, dark, or two high-contrast (AAA) themes.
- **Motion** — match system, or force reduced / full.
- **Text size & spacing** — scale the surface and line spacing.
- **Links & focus** — always-underline links and an enhanced focus ring.
- Everything is **opt-in** and **saved locally**.

## Theme & contrast

Pick **Light** or **Dark**, or one of the two **High contrast** themes. The
high-contrast themes meet the **WCAG AAA** contrast standard for text.

## Motion

Choose **Match system** to follow your operating system's "reduce motion"
setting, or force motion **Reduced** or **Full** regardless of the system
preference.

## Text size and line spacing

Scale the reading and writing surface up or down, and add extra space between
lines of prose for easier reading.

## Links and focus

Turn on **Always underline links** so links don't rely on colour alone. Turn on
the **Enhanced focus indicator** for a thicker, higher-contrast outline as you
move through the app with a keyboard.

## Keyboard and screen readers

- **Tab** / **Shift+Tab** move between controls.
- A **skip link** jumps straight to the main content.
- Controls carry labels and roles so screen readers announce them correctly.

## Your preferences are saved

Changes are stored locally and reapplied the next time you open the app.

## Related

- [Customisation & settings](customization-and-settings) — themes and typography.
- [Keyboard shortcuts](keyboard-shortcuts) — keyboard navigation.
