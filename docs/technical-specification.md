...
- **Manifest & icons.** A web app manifest (`name: LIpsum Writer`, `display: standalone`, relative `start_url`/`scope`/`id` so both deploy bases work) with 192/512 any-purpose icons, a 512 maskable icon, and an apple-touch icon. `theme-color` metas follow the light/dark colour scheme (paper/ink tokens).
